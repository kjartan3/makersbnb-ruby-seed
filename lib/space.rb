...
class SpaceRepository
  def all
    ...
    sql = 'SELECT id, name, description, available_start, available_end, price, user_id FROM users;'
  ...
  def find(id)
    sql = 'SELECT id, name, description, available_start, available_end, price, user_id FROM users WHERE id = $1;'
